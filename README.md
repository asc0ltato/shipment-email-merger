...
### AI и интеграции
- **Deepseek API** (через OpenRouter) — AI для анализа содержимого писем и извлечения структурированных данных
...
7. **AI анализ:**
   ...
   - Сформированный контекст отправляется в AI сервис (Google Gemini через OpenRouter API) через HTTP запрос с использованием специально разработанного промпта
   ...
   - Результат сохраняется в таблицу `summaries` с пометкой `is_ai_generated = true` и статусом pending (или failed при ошибке)
...
- **AI анализ (структурированные данные):**
  ...
  - Основные поля структуры:
    - `name` — название груза
    - `is_ai_generated` — флаг, указывающий что данные сгенерированы AI
    - `upload_id` — уникальный идентификатор загрузки
    - `pre_shipments` — массив предотгрузок, каждая содержит:
    ...
    - `modes` — массив видов перевозки (id, название)
...
- **Approved (Подтверждено)** — summary проверена и одобрена пользователем, готова для использования внешними сервисами через summary API. Только группы со статусом approved доступны через эндпоинт `/api/internal/email-groups/approved`. Пользователь может переключить rejected summary обратно в approved
...
2. Содержимое писем форматируется в специальный промпт и отправляется в AI модель через `DeepseekService` (используется OpenRouter API для доступа к Google Gemini)
...
4. Результат анализа преобразуется в JSON структуру `ShipmentRequest` со следующими полями:
   - `name` — название груза
   - `is_ai_generated` — флаг генерации AI (всегда `true`)
   - `upload_id` — уникальный идентификатор загрузки
   - `pre_shipments` — массив предотгрузок с полной информацией о датах, времени, адресах отправки и назначения, содержимом груза
...
- `aiAnalysis` (JSONB, NOT NULL) — структурированные данные AI анализа в формате JSON, хранятся в типе JSONB PostgreSQL для эффективного поиска и индексации. Содержит объект типа `ShipmentRequest` с полями: `name`, `is_ai_generated`, `upload_id`, `pre_shipments` (массив с адресами, датами, содержимым), `modes` (массив видов перевозки), `for_carriers` (дополнительная информация перевозчику)
...
aiAnalysis - структурированные данные AI анализа в формате JSONB\
...
4. AI сервисы (ai/)
- DeepseekService - интеграция с AI API для анализа контента:\
generateStructuredEmailGroupData() - отправляет содержимое писем в AI модель (Google Gemini через OpenRouter) и получает структурированные данные о грузе. Обрабатывает ошибки API (402, 429, 401, 5xx) отдельно от ошибок анализа данных\
...
**Summary API** — это отдельный REST API сервис, предназначенный для предоставления внешним системам доступа к структурированным JSON данным (aiAnalysis) approved AI анализов групп писем. Сервис работает как прокси между внешними клиентами и основным backend API, обеспечивая безопасный доступ только к одобренным структурированным данным о грузах.

### Основные возможности

- **Доступ к approved summary** — предоставляет только структурированные JSON данные (aiAnalysis) для групп писем с подтвержденным AI анализом
- **JWT аутентификация** — безопасная внутренняя аутентификация с backend API через JWT токены
- **Трансформация данных** — извлекает только `aiAnalysis` (JSON) из approved групп
...
4. **Backend API** проверяет JWT токен через middleware `requireInternalAuth`
   ...
   - Возвращает только группы со статусом summary = 'approved' со всеми данными (emails, attachments, aiAnalysis и т.д.)

5. **Summary API** получает данные от backend API и трансформирует их
   - Извлекает только `aiAnalysis` (структурированные JSON данные) из approved групп
   - Исключает все остальные данные (emails, attachments, userId, статусы и т.д.)
   - Возвращает только `emailGroupId` и `aiAnalysis` (JSON) для каждой группы

6. **Внешний клиент** получает ответ от Summary API с JSON данными о грузах (aiAnalysis)
...
**Примечание:** API возвращает только `aiAnalysis` (структурированные JSON данные) для каждой approved группы. Поле `is_ai_generated` исключено из ответа. Все остальные данные (emailGroupId, emails, attachments, userId, статусы, текстовое summary) также исключены из ответа
...
**SummaryService:**
- Извлекает только `aiAnalysis` (структурированные JSON данные) из approved групп, полученных от backend API
- Исключает все остальные данные (emails, attachments, userId, статусы, текстовое summary и т.д.)
- Возвращает только `emailGroupId` и `aiAnalysis` для каждой группы
- Фильтрует группы без `aiAnalysis` (группы без данных исключаются)

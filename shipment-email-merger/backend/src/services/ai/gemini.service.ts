    ...
    private createAnalysisPrompt(emails: IEmail[]): string {
        ...
        return `
           ...
           8. ГРУЗ (contents[].name и contents[].quantity):
            ...
            КЛЮЧЕВЫЕ СЛОВА: "груз", "товар", "наименование", "описание груза"
            ЧТО ИСКАТЬ:
            ...
            * Если количество не указано, но есть название груза - используй quantity: 1
            ...
            2. Даже если заказ ОТМЕНЕН - извлекай всю информацию (даты, адреса, груз и т.д.)
            3. Если информация отсутствует - используй null (без кавычек в JSON)
            4. НИКОГДА не используй дефолтные значения типа "00:00", "0000-00-00", "Unknown" - если информации нет, используй null
            5. ОБЯЗАТЕЛЬНЫЕ ПОЛЯ (не могут быть null):
            ...
            7. Не придумывай информацию - используй только то, что есть в письмах
            8. Если информации мало - верни JSON с тем, что нашел (null для остального, но обязательные поля должны присутствовать)
